...
extension MainChatView {
    ...
    func pageIndicator() -> some View {
        let items = windowManager.windows.map { window -> PageIndicatorView.WindowItem in
            ...
            return PageIndicatorView.WindowItem(id: window.id, name: name, isStreaming: isStreaming)
        }
...
private struct PageIndicatorView: View, Equatable {
    ...
    var body: some View {
        #if DEBUG
        let _ = DebugMetrics.log("PageIndicator", "render | windows=\(items.count) active=\(activeIndex)")
        #endif
        HStack(spacing: DS.Spacing.s) {
            ForEach(Array(items.enumerated()), id: \.element.id) { index, item in
                ...
                Button {
                    ...
                    windowIndicatorLabel(name: item.name, isActive: isActive, isStreaming: item.isStreaming)
    ...
    @ViewBuilder
    private func windowIndicatorLabel(name: String, isActive: Bool, isStreaming: Bool) -> some View {
