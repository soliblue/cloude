...
extension ChatMarkdownParser {
    static func parseBoldItalic(
        _ remaining: inout Substring, font: Font?
    )
        -> [ChatMarkdownInlineSegment]?
    {
        ...
        return applyIntent(
            parseLineToSegments(innerText, font: font),
            intents: [.stronglyEmphasized, .emphasized])
    }

    static func parseStrikethrough(
        _ remaining: inout Substring, font: Font?
    )
        -> [ChatMarkdownInlineSegment]?
    {
    ...
    static func parseFilePath(
        _ remaining: inout Substring, font: Font?
    )
        -> ChatMarkdownInlineSegment?
    {
        if !(remaining.hasPrefix("/Users/") || remaining.hasPrefix("/tmp/")
            || remaining.hasPrefix("/var/"))
        {
            return nil
        }
        var pathText = ""
        ...
        if looksLikeFilePath(pathText) { return .filePath(pathText) }
        var attr = AttributedString(pathText)
        if let font = font { attr.font = font }
        return .text(attr)
    }

    static func parseLink(_ remaining: inout Substring, font: Font?) -> ChatMarkdownInlineSegment? {
        if !remaining.hasPrefix("[") { return nil }
    ...
    static func applyIntent(
        _ segments: [ChatMarkdownInlineSegment], intents: InlinePresentationIntent
    ) -> [ChatMarkdownInlineSegment] {
