...
struct OpenHarnessTests {
    @MainActor static func main() async throws {
        if CommandLine.arguments.count == 3 {
            try await ForkRetryTests.stage(
                CommandLine.arguments[1], directory: URL(fileURLWithPath: CommandLine.arguments[2]))
        } else {
            ...
            let importResult = await SessionRemoteService.open(
                thread, endpoint: endpoint, store: store, context: context)
