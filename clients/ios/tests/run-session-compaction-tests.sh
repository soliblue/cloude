...
swiftc -parse-as-library -target "$(uname -m)-apple-macosx14.0" \
  ...
  src/Features/Sessions/Logic/SessionCompaction*.swift \
  src/Features/Chat/Logic/Chat{Goal,Model,Provider,Effort,PermissionMode}.swift \
  tests/session-compaction/*.swift -o "$test_dir/session-compaction-tests"
"$test_dir/session-compaction-tests"
