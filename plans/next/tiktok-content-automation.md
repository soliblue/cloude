...
## Learnings from Oliver Henry's System

### Hook Formula (THE key insight)
- **What works**: `[Another person] + [conflict/doubt] → showed them AI → they changed their mind`
...
- **What fails**: Self-focused, feature-focused hooks
  - "See your room in 12+ styles before you commit" → 879 views
  - "The difference between $500 and $5000 taste" → 2,671 views
- **Rule**: Every hook needs another person and a conflict. If there isn't one, it won't work.

### Slideshow Format
- 6 slides exactly (TikTok's sweet spot)
- Portrait orientation: 1024x1536 (NOT landscape — black bars kill engagement)
...
### Image Generation
- Use "iPhone photo" and "realistic lighting" in prompts → looks like real phone photos, not AI art
- **Lock the architecture**: Write one obsessively detailed room/scene description, copy-paste into every prompt. Only change the style between slides. Otherwise windows move, furniture changes size, looks fake.
- "Before" images need signs of life — mugs, remote controls, flat screen TV. Empty rooms look like show homes, nobody relates.
- Don't add people to generated images — doesn't work well.
- Cost: ~$0.50/post real-time, ~$0.25 with Batch API

### Text Overlays
- Font size 6.5% (not 5% — too small)
- Position below TikTok's status bar area
- Watch for canvas compression — long lines get squashed horizontally

### Posting Workflow
1. Agent generates images, adds text overlays, writes caption
2. Upload to TikTok as draft via API (privacy_level: SELF_ONLY)
3. Human adds trending sound manually (can't be automated via API)
4. Human pastes caption and publishes (~60 seconds)
5. Run on cron jobs at peak engagement times

### Skill File Philosophy (mirrors Cloude's .claude/skills/)
- Skill files = markdown docs teaching the agent specific workflows
- His TikTok skill file is 500+ lines, rewritten ~20 times in the first week
- Every failure becomes a rule, every success becomes a formula
- The agent compounds — gets genuinely better than the human at the task
- "The agent is only as good as its memory"

### Results
- 500K+ views in under a week
- 4 posts over 100K views
- 108 paying subscribers
- ~$588/month MRR
- Cost: ~$0.50/post in API calls

### Tools Used
- OpenClaw (open-source Claude Code alternative) — the agent runtime
- GPT image generation (gpt-image-1.5) — slideshow images
- Postiz — social media scheduling with API for TikTok draft uploads
- RevenueCat — subscription analytics fed back to the agent
- Batch API — 50% cheaper for overnight pre-generation

## Implementation Ideas for Cloude
- `/tiktok` skill that generates slideshows from a product + hook
- Image generation via existing `/image` skill (Gemini) or add OpenAI image API
- Text overlay via Sharp/Canvas in a script
- Postiz integration for draft uploads
- Hook brainstorming mode that enforces the "person + conflict" formula
- Performance tracking in memory files
...
- Access to post to TikTok via Postiz, i chose this because it has super easy docs for Larry to understand, and the API is included in the price.
...
1. **A Computer** - It doesn't need to be powerful. Larry is an old gaming PC with Ubuntu, which is overkill. Almost any spare computer, a Raspberry Pi, or a cheap VPS will work. You Don't have to be trendy and buy a brand new Mac Mini. The minimum requirements to run openclaw are:
...
3. **Postiz** - this is how your agent posts to TikTok. It has an API that lets you upload slideshows as drafts.
...
1. **Get a machine running Linux.** Any old computer, a Raspberry Pi, or a cheap VPS, A Mac Mini if you're flash. Install Ubuntu if you're not sure what to pick.
...
3. **Get an image generation key.** I use OpenAI. Sign up at platform.openai.com. You'll use gpt-image-1.5 for image generation. Expect to spend about $0.50 per slideshow, or $0.25 if you use the Batch API.

4. **Sign up for Postiz.** This is the tool that connects your agent to TikTok. It has an API that lets you upload slideshows as drafts.
