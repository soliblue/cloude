---
title: "Heartbeat Refresh Button"
description: "Add a refresh button to the heartbeat sheet toolbar that re-fetches messages."
created_at: 2026-02-07
tags: ["heartbeat", "ui"]
icon: arrow.clockwise
build: 48
---


# Heartbeat Refresh Button {arrow.clockwise}
## Problem
No way to manually refresh/sync heartbeat messages if something was missed.

## Solution
Add a refresh button to the heartbeat sheet toolbar that re-fetches messages.

## Files to Change
- `HeartbeatSheet.swift` — add toolbar button
