# Turing Machine Website

Deploy interactive Turing machine visualization to turing.soli.blue via Cloudflare Pages.

## Done
- [x] Created project at `/Users/soli/Desktop/CODING/turing/`
- [x] Build script generates single self-contained `dist/index.html`
- [x] React component bundled with esbuild, React loaded from CDN

## To Test
- [ ] Verify it works in browser (play/pause/step/reset, speed control, input)
- [ ] Add subdomain `turing.soli.blue` in Cloudflare
- [ ] Connect to Cloudflare Pages and deploy
