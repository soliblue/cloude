...
DAY_START=$(date -j -f "%Y-%m-%d %H:%M:%S" "$TARGET_DATE 00:00:00" +%s 2>/dev/null)
if [ -z "$DAY_START" ]; then
    echo "ERROR: Invalid date format. Use YYYY-MM-DD"
    exit 1
fi
DAY_END=$((DAY_START + 86400))
DAY_START_MAC=$((DAY_START - MAC_EPOCH))
DAY_END_MAC=$((DAY_END - MAC_EPOCH))

echo "App usage for $TARGET_DATE"
echo "---"
...
SELECT
    ...
    CAST(ROUND(SUM(
        CASE
            ...
            THEN MIN(ZENDDATE, $DAY_END_MAC) - MAX(ZSTARTDATE, $DAY_START_MAC)
            ELSE 0
        END
    ) / 60.0) AS INTEGER) AS total_minutes
FROM ZOBJECT
WHERE ZSTREAMNAME = '/app/usage'
    AND ZENDDATE >= $DAY_START_MAC
    AND ZSTARTDATE < $DAY_END_MAC
