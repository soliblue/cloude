...
Use these references when the question is about code structure rather than distributed systems:
- Read `references/design-patterns.md` when choosing or comparing patterns such as Strategy, Factory, Adapter, Facade, Observer, State, State Machine, Command, Registry, Ports and Adapters, DI, reducer-style state, MVC, or MVVM.
- Read `references/anti-patterns.md` when the user is unsure whether a pattern is justified, or when code smells suggest over-abstraction, boolean soup state, stringly typed routing, global state, inheritance misuse, or god objects.
