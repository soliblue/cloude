...
- **User ID**: xQqqZqlgcoSxTgPe03BiXmVE2JJ2
...
```bash
curl -sL "https://api.manifold.markets/v0/market/{marketId}/positions?userId=xQqqZqlgcoSxTgPe03BiXmVE2JJ2" | jq
```
...
```bash
curl -sL "https://api.manifold.markets/v0/search-markets?term=&sort=newest&filter=open&creatorId=xQqqZqlgcoSxTgPe03BiXmVE2JJ2&limit=20" | jq -r '.[].id'
```
