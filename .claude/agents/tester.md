...
| `app_ref` | Readiness info: sim udid, bundle id, log path, build ref |
...
Scenarios assume the app is already running and ready (`app_ref` came from the launcher). Before executing each scenario's steps:
