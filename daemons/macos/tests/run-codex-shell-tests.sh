...
TEMP="$(mktemp -d /tmp/afto-codex-shell.XXXXXX)"
trap 'rm -rf "$TEMP"' EXIT
chmod +x "$ROOT/tests/fake-codex-shell.py"
swiftc "$ROOT/src/DaemonLifecycle.swift" "$ROOT/src/Runner.swift" "$ROOT/src/ClaudeSubscriptionPolicy.swift" \
  ...
  "$ROOT/src/CodexSessionStore.swift" "$ROOT/src/RunnerManager.swift" "$ROOT/src/ImageDropbox.swift" \
  ...
  "$ROOT/tests/CodexShellTests.swift" -o "$TEMP/shell-tests"
CLOUDE_DATA="$TEMP/data" CODEX_HOME="$TEMP/codex" "$TEMP/shell-tests" "$ROOT/tests/fake-codex-shell.py" "$TEMP/codex"
