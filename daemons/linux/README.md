...
Task lifecycle safeguards reject malformed archive flags and refuse to archive active native tasks, including activity reached through another local session. New turns also reject known remote activity and active resume snapshots instead of taking over a running imported task. With `codexActiveFork`, side chats can fork while the source continues running: the daemon reads native history, pins `lastTurnId` to the final finished turn before any unfinished turn, and returns the authoritative `thread` snapshot. The fork requests `deferGoalContinuation:true` and `excludeTurns:false`, carrying a goal without starting its automatic continuation; these experimental fields are verified in Codex 0.153.4 with `generate-json-schema --experimental`. Completed, interrupted and failed turns form finished history. A task without a finished turn returns 409; no source interruption or implicit steering occurs. Returned turn IDs must exactly match the selected finished prefix, in order, before a side chat is saved. With `codexIdempotentFork`, `newSessionId` is a durable attempt identity: reuse it with the same source and requested path to recover the original fork and snapshot after a lost response or daemon restart. A fsynced pending intent precedes the native fork, and the completed snapshot and mapping are durable before success. Concurrent identical attempts return 409 with `code:fork_pending` and `retriable:true`; changed inputs conflict. An ambiguous provider outcome returns `code:fork_outcome_unknown` and `retriable:false` and is never issued again. Error text is human-readable. Receipts are private and retained without an unsafe expiry. Clients must display the returned history rather than copying the source chat. Imports and forks reserve their target session until the native response is validated and saved; concurrent starts or mapping changes return 409. Session lookup is case-insensitive, matching journal identity. External close, archive, or deletion notifications clear pending approvals and finish a live stream instead of leaving its heartbeat running. An omitted reasoning-effort override preserves the value returned by native thread resume. Afto still exposes archival rather than permanent native thread deletion. Codex 0.153.4 can omit a newly forked task from native task lists until it receives its first new message, even though direct history and archive/restore by ID work. Saved side chats remain accessible from phone history; Afto does not rewrite Codex’s private database to change this behavior.
